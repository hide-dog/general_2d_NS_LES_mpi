...
function set_mpi(cellxmax, cellymax)
    ...
    num_div = 0.0      # 各方向の分割数
    if floor(comm^0.5 * 10000) % 10000 != 0 || comm <= 0.0
        println("------------------------\n")
        println(" mpi error \n")
    ...
    =#
    cellNx, ista, iend, cellNy, jsta, jend = start_and_end0(Nx, Ny, num_div)

    return cellNx, ista, iend, cellNy, jsta, jend, num_div, rank
end

# ---------------------------------------------
# mpi division
# ---------------------------------------------
function start_and_end0(Nx, Ny, num_div)
    ...
    for l in 1:nval
        for i in is:ie
            for j in js:je
                ...
                volume[i,j,l] = volume_all[ii, jj, l]
                wally[i,j,l]  = wally_all[ii, jj, l]
                dx[i,j,l]     = dx_all[ii, jj, l]
                dy[i,j,l]     = dy_all[ii, jj, l]
            end
        end
    end
...
function Conversion_to_coordinate_numbers(i, j, cellx_mpi, celly_mpi, num_div, rank)
    ...
    div system
      ...
      rank      div(rank, comm^0.5)    rem(rank, comm^0.5)
...
function mpi_div_3d(ite, ls, le, js, je, is, ie)
    #= ite: iteration number (ista, iend generated by start_and_end(N,comm))
